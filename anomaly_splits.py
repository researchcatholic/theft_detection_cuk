...
#    classifier = LOF(n_neighbors=10, contamination=0.1)
...
for dt in rrule(DAILY, dtstart=start_date, until=end_date):
    if (data.loc[dt.strftime("%Y-%m-%d")]['value']).values.size != 0:
        data.loc[dt.strftime("%Y-%m-%d"),'score'] =clf.predict(preprocessing.normalize(data.loc[dt.strftime("%Y-%m-%d")].value.values.reshape(1, -1)))
        data.loc[dt.strftime("%Y-%m-%d"),'probab'] =(clf.predict_proba((preprocessing.normalize(data.loc[dt.strftime("%Y-%m-%d")].value.values.reshape(1, -1))))[0][0])
        for _ in range (0,splits):
            data.loc[dt.strftime("%Y-%m-%d"),'score_cla{}'.format(_+1)] = cla[_].predict(preprocessing.normalize(data.loc[dt.strftime("%Y-%m-%d")].value.values.reshape(1, -1))[0][_*(144//splits):(_+1)*144//(splits)].reshape(1,-1))
        i = i + 1
    else:
#        print(dt)
        continue
data.loc[:,['value','score']].plot(figsize=(144, 4))
#data.loc[:,['value','probab']].plot(figsize=(144, 4))
for dt in rrule(DAILY, dtstart=start_date, until=end_date):
    ...
    if (data.loc[dt.strftime("%Y-%m-%d")]['value']).values.size != 0:
        ...
        for _ in range (0,splits):
            if data.loc[dt.strftime("%Y-%m-%d")]['score_cla{}'.format(_+1)].all() == 1:
                plt.axvspan( dt+ timedelta(days=(_)/splits), dt+ timedelta(days=(_+1)/splits), color='blue', alpha=0.2)
    else:
        continue

LOFS = np.unique(data[data.score == 1].index.date.astype(str))
print("LOCAL OUTLIER FACTOR:",len(LOFS))
print(LOFS)
print("number of suspicious days for",splits," splits is",len(LOFS))

#%%
data['doubt'] = (data['score']+data['score_cla1']+data['score_cla2']+data['score_cla3'])
data['doubt'] = np.where(data['doubt']>0,1,0)
#%%
data.loc[:,['value','doubt']].plot(figsize=(144, 4))
#%%
#(clf.predict_proba((preprocessing.normalize(data.loc[dt.strftime("%Y-%m-%d")].value.values.reshape(1, -1))))[0][0])
#%%
data.loc[:,['value','score','probab']].plot(figsize=(144, 4))
